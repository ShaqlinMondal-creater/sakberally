...
<!-- Update -->
 <script>
  ...
  async function updateProduct(productId) {
    ...
    // Fetch the current row data for prefill (we already have it in DOM)
    const row = findRowByProductId(productId);
    const nameCell = row?.querySelector('td:nth-child(1) .font-medium')?.textContent?.trim() || '';
    const priceCell = row?.querySelector('td:nth-child(3)')?.textContent?.replace('₹','').trim() || '';
    const categoryCell = row?.querySelector('td:nth-child(2)')?.textContent?.trim() || '';

    // SweetAlert popup for edit
    const { value: formValues } = await Swal.fire({
      title: 'Update Product',
      html: `
        <input id="swal-name" class="swal2-input" placeholder="Product Name" value="${nameCell}">
        <input id="swal-price" type="number" class="swal2-input" placeholder="Price" value="${priceCell}">
        <input id="swal-category" class="swal2-input" placeholder="Category ID (optional)">
        <input id="swal-brand" class="swal2-input" placeholder="Brand ID (optional)">
      `,
      focusConfirm: false,
      ...
      preConfirm: () => {
        return {
          ...
          category_id: document.getElementById('swal-category').value ? Number(document.getElementById('swal-category').value) : null,
          brand_id: document.getElementById('swal-brand').value ? Number(document.getElementById('swal-brand').value) : null,
        };
      }
    });
    ...
    Object.keys(payload).forEach(k => payload[k] === null || payload[k] === '' ? delete payload[k] : 0);
